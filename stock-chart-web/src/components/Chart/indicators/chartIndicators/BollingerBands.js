...
const BollingerBands = {
      ...
      <SettingsContainer>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
          <button 
            ...
            onClick={() => setTempSettings({...BollingerBands.defaultSettings})}
        ...
        {activeTab === 'variables' && (
          <>
            ...
            <SettingRow>
              <SettingLabel>승수</SettingLabel>
              <SettingInput
                type="number"
                value={tempSettings.multiplier}
                onChange={(e) => handleSettingChange('multiplier', e.target.value)}
                min="0.5"
                max="4"
                step="0.1"
                style={{ textAlign: 'left', width: '60px' }}
  ...
  calculate: (data, settings) => {
    return calculateBollingerBands(data, settings.period, settings.multiplier);
  },
  
  // 차트에 시리즈 추가 함수
  addToChart: (chart, data, settings) => {
    const { middle, upper, lower } = BollingerBands.calculate(data, settings);
    const series = {};
    ...
    const upperSeries = chart.addLineSeries({
      ...
      title: `BB Upper(${settings.period}, ${settings.multiplier})`,
    });
    ...
    const lowerSeries = chart.addLineSeries({
      color: settings.color || '#2962FF',
      lineWidth: settings.bandLineWidth || 1,
      title: `BB Lower(${settings.period}, ${settings.multiplier})`,
    });
    ...
    if (settings.showBackground !== false) {
      ...
      // 영역 색상 설정
      const colorWithOpacity = `${settings.color || '#2962FF'}20`;
  ...
  removeFromChart: (chart, series) => {
    ...
    try {
      ...
      console.warn('볼린저 밴드 시리즈 제거 중 오류 발생:', error);
    }
  }
};
