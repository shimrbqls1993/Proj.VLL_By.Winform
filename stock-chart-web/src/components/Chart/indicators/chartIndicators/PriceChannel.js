...
import { calculatePriceChannel } from '../../utils/indicatorCalculations';
import { 
  SettingsContainer, 
  SettingTitle, 
  SettingRow, 
...
const PriceChannel = {
  ...
  SettingsComponent: ({ settings, onSettingsChange }) => {
    ...
    const handleSettingChange = (field, value) => {
      setTempSettings({
        ...tempSettings,
        [field]: field === 'period' || field === 'centerLineWidth' || field === 'bandLineWidth' ? parseInt(value) : 
      ...
      <SettingsContainer>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
          <button 
            ...
            onClick={() => setTempSettings({...PriceChannel.defaultSettings})}
        ...
        {activeTab === 'style' && (
          <>
            ...
            <SettingRow>
              <SettingLabel>중심선 두께</SettingLabel>
              <select
                value={tempSettings.centerLineWidth}
                onChange={(e) => handleSettingChange('centerLineWidth', e.target.value)}
                style={{
                  padding: '2px 5px',
                  border: '1px solid #ddd',
                  borderRadius: '4px',
  ...
  calculate: (data, settings) => {
    return calculatePriceChannel(data, settings.period);
  },
  
  // 차트에 시리즈 추가 함수
  addToChart: (chart, data, settings) => {
    const { upper, middle, lower } = PriceChannel.calculate(data, settings);
    const series = {};
    
    // 중간선
    const middleSeries = chart.addLineSeries({
      color: settings.color || '#00BCD4',
      lineWidth: settings.centerLineWidth || 1,
      title: `PC(${settings.period})`,
    });
    ...
    const upperSeries = chart.addLineSeries({
      ...
      lineStyle: 0, // 실선
      title: `PC Upper(${settings.period})`,
    });
    ...
    const lowerSeries = chart.addLineSeries({
      ...
      lineWidth: settings.bandLineWidth || 1,
      lineStyle: 0, // 실선
      title: `PC Lower(${settings.period})`,
    });
    ...
    if (settings.showBackground !== false) {
      ...
      // 영역 색상 설정
      const colorWithOpacity = `${settings.color || '#00BCD4'}30`;  // 30 = 약 19% 불투명도
  ...
  removeFromChart: (chart, series) => {
    ...
    try {
      ...
      if (series.area && chart) {
        chart.removeSeries(series.area);
      }
    } catch (error) {
      console.warn('가격 채널 시리즈 제거 중 오류 발생:', error);
    }
  }
};
